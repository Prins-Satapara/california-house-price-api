...
class HouseFeatures(BaseModel):
    MedInc: float = Field(gt=0, description="Median income of households in the block")
...
def home():
    return {
        "massage" : "California house price prediction API", 
        "status" : "running", 
...
async def predict_file(file: UploadFile = File(...)):
    ...
    if not file.filename or not file.filename.endswith(".csv"):
        raise HTTPException(
            status_code=400,
            detail="Please upload a csv files only"
        )
    ...
    try:
        ...
        # df["PredictedPriceUSD"] = df["PredictedPrice"].apply(lambda x: f"${x:,.0f}")
